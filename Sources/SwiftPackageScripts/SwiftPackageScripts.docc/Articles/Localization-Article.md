...
SwiftPackageScripts has an `l10n-gen.sh` script that can generate public, namespaced key wrappers from any Xcode 26 string catalog.
...
The key identifiers that Xcode 26 auto-generates are however internal, which means that they can't be accessed from other packages or targets. The `scripts/l10n-gen.sh` script can therefore generate public key wrappers for these internal keys.


## Running the script

The `scripts/l10n-gen.sh` script can be called with the `--from/--to` or `--package/--catalog/--target` options. Use from and to to use any call-site relative paths, and the package approach to use a catalog and target file within a certain package.
...
Have a look at the `l10n-gen.sh` file in the project root to see how you can add a shortcut script for your own package. 


## Namespaces

The `scripts/l10n-gen.sh` script will create a new namespace for each period you add to your keys, and will by default use `l10n` as the root namespace. 
...
```bash
./scripts/l10n-gen.sh --from ... --to ... --root myPackage
```
